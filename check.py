...
Đã bổ sung: Checkbox Tự động Kiểm tra (Auto Check) để tự động reset camera sau khi xử lý.
"""
...
def main_app(credentials):
    ...
    # --- BỔ SUNG: CHECKBOX AUTO CHECK ---
    auto_check_checked = st.checkbox(
        "Tự động Kiểm tra (Auto Check)",
        value=False, # Mặc định TẮT
        help="Nếu được bật, ứng dụng sẽ tự động xóa ảnh đã chụp và bắt đầu lại sau 5 giây (sau khi xử lý xong), không cần nhấn 'Clear Photo'."
    )
    # ----------------------------------
            ...
            with result_placeholder.container():
                ...
                if stt_match and distance is not None: # Đảm bảo cả stt_match và distance đều có giá trị
                    ...
                    * **Độ tương đồng (Khoảng cách Cosine):** `{distance:.4f}`
                    ...
                    # --- LOGIC TỰ ĐỘNG CLEAR/RERUN (THAY ĐỔI: DÙNG delay_time) ---
                    delay_time = 5 if auto_check_checked else 2 # 5s nếu Auto Check, 2s nếu không
                    st.info(f"🔄 Tự động reset camera sau **{delay_time}** giây...")
                    ...
                    # --- LOGIC RERUN KHI AUTO-CHECK BẬT (KHÔNG KHỚP) ---
                    if auto_check_checked:
                        st.info("🔄 Đang ở chế độ Tự động Kiểm tra. Tự động reset camera sau **5** giây...")
                        time.sleep(5)
                        if TEMP_IMAGE_PATH and os.path.exists(TEMP_IMAGE_PATH):
                            os.remove(TEMP_IMAGE_PATH)
                        st.session_state['camera_input_key'] += 1 
                    ...
                    # --- LOGIC RERUN KHI AUTO-CHECK BẬT (NHIỀU KHUÔN MẶT) ---
                    if auto_check_checked:
                        st.info("🔄 Đang ở chế độ Tự động Kiểm tra. Tự động reset camera sau **5** giây...")
                        time.sleep(5)
                        if TEMP_IMAGE_PATH and os.path.exists(TEMP_IMAGE_PATH):
                    ...
                    # --- LOGIC RERUN KHI AUTO-CHECK BẬT (KHÔNG KHUÔN MẶT) ---
                    if auto_check_checked:
                        st.info("🔄 Đang ở chế độ Tự động Kiểm tra. Tự động reset camera sau **5** giây...")
                        time.sleep(5)
                        if TEMP_IMAGE_PATH and os.path.exists(TEMP_IMAGE_PATH):
    ...
    if CHECKLIST_SESSION_KEY in st.session_state:
        update_checklist_display(checklist_placeholder, st.session_state[CHECKLIST_SESSION_KEY])# check.py
"""
Chứa các hàm xử lý DeepFace, OpenCV, logic cập nhật checklist và giao diện Streamlit.
Đã bổ sung: Checkbox để điều khiển việc hiển thị ảnh đã cắt và ảnh dataset/không khớp.
Đã bổ sung: Checkbox Tự động Kiểm tra (Auto Check) để tự động reset camera sau khi xử lý.
