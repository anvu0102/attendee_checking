...
def main_app(credentials):
    ...
    # Khởi tạo key cho Auto Check nếu chưa có
    ...
    with col_auto:
        ...
        auto_check = st.checkbox(
            ...
            help="Tự động reset camera sau 5 giây xử lý để điểm danh liên tục. (Không phải livestream thực sự)."
        )
        ...
        if captured_file is not None:
            ...
            with result_placeholder.container():
                ...
                if stt_match and distance is not None: 
                    ...
                    # --- HIỂN THỊ ẢNH ĐÃ CẮT VÀ ẢNH DATASET TRÙNG KHỚP (CÓ ĐIỀU KIỆN) ---
                    ...
                    * **Độ tương đồng (Khoảng cách Cosine):** `{distance:.4f}`
                    ...
                    # --- HIỂN THỊ ẢNH ĐÃ CẮT (CÓ ĐIỀU KIỆN) ---
                    if show_debug_images: 
                        if TEMP_IMAGE_PATH:
                ...
                if should_auto_rerun:
                    # Cập nhật checklist display trước khi rerun nếu có thay đổi
    ...
    # 4. HIỂN THỊ TRẠNG THÁI CHECKLIST BAN ĐẦU HOẶC SAU KHI XỬ LÝ (NẾU KHÔNG AUTO RERUN)
    if CHECKLIST_SESSION_KEY in st.session_state:
        # Nếu chưa có ảnh chụp hoặc đã xử lý xong và không auto rerun, thì hiển thị checklist
        update_checklist_display(checklist_placeholder, st.session_state[CHECKLIST_SESSION_KEY])
